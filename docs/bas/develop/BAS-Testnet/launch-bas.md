...
Before running the command you must do the following steps:
* Ensure you have a dedicated machine that has at least 8 dedicated CPU cores and 32GB RAM, as it needs to run 7 nodes.
...
* `validators` — list of initial validator set (make sure that you have the same list in the docker compose file).
* `systemTreasury` — address of system treasury that accumulates 1/16 of rewards (might be governance).
   ...
   * `felonyThreshold` — after missing this amount of blocks per day validator goes to jail for N epochs.
   ...
   * `minValidatorStakeAmount` — how many tokens the validator must stake to create a validator (in ETH).
...
* `faucet` — a map with initial balances for the faucet and other needs.
