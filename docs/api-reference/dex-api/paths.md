...
---
##### ***GET***
**요약:** Get the block time.

**설명:**  Gets the latest block time and the current time according to the HTTP service.
...
**메인넷 URL:** [https://dex.binance.org/api/v1/klines?symbol=NNB-338_BNB&interval=5m](https://dex.binance.org/api/v1/klines?symbol=NNB-338_BNB&interval=5m)
...
**Note:**해당 엔드포인트가 곧 제거되며 [this api](https://docs.binance.org/api-reference/dex-api/block-service.html#apiv1txs) 를 대신 사용하세요 (이전 방법은 가이드를 읽어주세요)..
...
---
**Note:**해당 엔드포인트가 곧 제거되며 [this api](https://docs.binance.org/api-reference/dex-api/block-service.html#apiv1blocksblockheighttxs) 를 대신 사용하세요 (이전 방법은 가이드를 읽어주세요)..
...
| 코드 | 설명 | 스키마 |
| ---- | ----------- | ------ |
| 200 | Success(성공) | [Markets](#markets) |
| 400 | Bad Request(잘못된 요청) | [에러](#에러). |
